...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Get current item
    const currentItem = await db.perangkatDesa.findUnique({
      where: { id }
    });
    ...
    const allItems = await db.perangkatDesa.findMany({
      orderBy: { urutan: 'asc' }
    });
    ...
    console.error('Error reordering Perangkat Desa:', error);
    return NextResponse.json(
